    ...
    <>
      <section className="bg-slate-100 w-full flex justify-center font-medium text-sm">
        <nav className="lg:w-4/5 2xl:w-3/5 py-2 text-slate-950 flex justify-between">
          <ul className="flex space-x-10">
            ...
            <li className="flex">
              <CheckMarkGreen /> Direct contact: 0317 765 005
            </li>
          </ul>
          <a href="https://www.5sterrenspecialist.nl/keurmerkhouders/nudetuin" aria-label="Bekijk onze reviews via de 5sterrenspecialist" target="_blank"><span className="hover:underline">Bekijk onze reviews</span></a>
        </nav>
      </section>
      <header className="bg-[--wit] text-[--menu-tekst] h-fit w-full flex flex-col justify-center items-center sticky top-0 z-50">
        <nav className="lg:w-4/5 2xl:w-3/5 grid grid-cols-3 space-between items-center">
          ...
          <form
            action={search}
            className="col-start-2 col-span-full w-2/3 flex flex-col relative"
          >
            ...
            <input
              ...
              name="zoeken"
              type="text"
              placeholder="Bijvoorbeeld: Cera3line Lux & Dutch"
              className="shadow-md bg-slate-100 rounded-lg py-2 px-5 hover:shadow-lg"
              defaultValue={zoekTerm ? zoekTerm : undefined}
            />
            <button
              aria-label="Zoeken"
              className="hover:shadow-xl hover:scale-95 bg-[--primary] text-white w-fit py-2 px-7 rounded-lg absolute right-0 bottom-0 font-semibold"
            >
              Zoeken
            </button>
          </form>
